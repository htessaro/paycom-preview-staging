...
|---|---|---|
| `destination` | Yes (or `destination_data`) | The ID of a stored payment method (`pm_card_...` or `pm_us_bank_account_...`). |
| `destination_data` | Yes (or `destination`) | Raw payment details for the destination (card, network token, or bank account). Requires PCI-DSS for raw card data. |
| `payout_method` | Yes (or `payout_method_data`) | The ID of a stored payout method. |
| `payout_method_data` | Yes (or `payout_method`) | Raw payout method details. |
...
## Retrieve a payout by reference

You can look up a payout by the `reference` string you provided at creation time using
`GET /v1/payouts/reference/{reference_id}`. This is useful for reconciliation when you know
your internal reference but not the `po_...` ID.
